...
//这里依照上面那样写编译时是会报错的，报错信息，Module not found: Error: Can’t resolve ‘element-plus/lib/theme-chalk/index.css’，改成下面这样就OK了
...
上面是 coderwhy 当时的做法，现在官网推荐下面这个做法
...
2.配置 webpack.config.js，这里直接配置 vue.config.js
...
3.如果，`unplugin-auto-import` 起作用了，那么`auto-imports.d.ts`文件，应该会有相应的记录
...
1.当你必须要引入样式时，如下，使用`ElMessage`时，那就必须要安装`unplugin-element-plus`插件，否则样式会无法自动引入。
...
3.配置`webpack.config.js`，`Vue-CLI`这里直接配置`vue.config.js`
...
但是这里依然有个弊端：（这个弊端应该是没有了，因为我们不需要手动引入组件了）
